...
export const Employee = ({employee, deleteEmployee}) => {
  ...
  return (
    ...
    <td className="text-right px-6 py-4 whitespace-nowrap font-medium text-sm">
        <a  onClick={(e, id) => editEmployee(e, employee.id)} className="text-indigo-500 hover:text-indigo-900 px-4 hover:cursor-pointer">Edit</a>
        <a onClick={(e,id) => deleteEmployee(e,employee.id)} className="text-indigo-500 hover:text-indigo-900 px-4 hover:cursor-pointer">Delete</a>
        </td>
</tr>
  )
}
